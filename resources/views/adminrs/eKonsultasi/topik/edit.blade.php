          ...
          <div class="card-header">
            <div class="row">
                ...
                <div class="col-lg-6 d-flex justify-content-end">
                    <a href="{{ url('adminRs/topik-eKonsultasi') }}" class="btn btn-sm btn-primary">Kembali</a>
                </div>
            </div>
        </div>
            {{-- otomatis masuk ke adminRs/daerah , karena pake resource --}}
            <form action="{{ url('adminRs/topik-eKonsultasi') }}/{{ $id }}" method="POST">
                        ...
                        <div class="mb-3">
                            <div class="form-group">
                                ...
                                <select class="form-control form-control-lg" id="exampleFormControlSelect1"  name="status" required>
                                    <option value="{{ $topik->status }}" selected>
                                        @if($topik->status == 1)
                                            Aktif
                                        @elseif ($topik->status == 2)
                                            Tidak Aktif
                                        @endif
                                    </option>
