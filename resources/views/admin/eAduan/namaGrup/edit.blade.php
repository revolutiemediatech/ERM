            ...
            <form action="{{ url("$url") }}/{{ $id }}" method="POST">
                ...
                <div class="card-body">
                    <div class="form-group">
                        <input type="hidden" id="i-nama" class="form-control" name="id" value="{{ $aduanGrup->id }}">
                        <div class="mb-3">
                            <label class="col-form-label pt-0" for="nama">Mitra</label>
                            <input name="nama" id="nama" class="form-control" type="text" value="{{ $aduanGrup->faskes->nama }}" disabled>
                        </div>
                        <div class="mb-3">
                            <label class="col-form-label pt-0" for="nama">Nama</label>
                            <input name="nama" id="nama" class="form-control" type="text" value="{{ $aduanGrup->nama }}">
                        </div>
                        <div class="mb-3">
                            <div class="form-group">
                                <label>Status</label>
                                <select class="form-control form-control-lg" id="exampleFormControlSelect1"  name="status" required>
                                    <option value="{{ $aduanGrup->status }}" selected>
                                        @if($aduanGrup->status == 1)
                                            Aktif
                                        @elseif ($aduanGrup->status == 2)
                                            Tidak Aktif
                                        @endif
                                    </option>
