...
namespace App\Http\Controllers\Admin\PengajuanObat;
...
{
    private $views      = 'admin/pengajuan-obat/rajal';
    private $url        = 'admin/pengajuan-obat-rajal';
    ...
    {
        $pasien   = $this->mPasien->where('status', 5)->get();
    ...
    {   
        if ($request->has('nama_obat1')) {
            $pasien             = $this->mPasien->where('id', $id)->first();
            // DB::table('pengajuan_obat')->where('idMPasien', $id)->delete();
            for ($i = 0; $i < count($request->nama_obat1); $i++) {
                $dataPengObat[] = [
                    'idObat'        => $request->nama_obat1[$i],
                    'dosis'         => $request->dosis1[$i],
        ...
        $request->has('tepat_indikasi') && $request->has('tidak_ada_polifarmasi')){
            ...
            return redirect("$this->url")->with('sukses', 'Data Pengajuan Obat berhasil di Verfikasi');
        }
    }
